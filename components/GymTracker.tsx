...
import {
  ...
  RANK_TIERS, calculateStreak, LeaderboardEntry,
} from '../gamification';
import {
...
import AnatomyViewer, { getViewForMuscle } from './Anatomy/AnatomyViewer';
...
const ExerciseBrowser: React.FC<{
  ...
  const filteredByMuscle = useMemo(() => {
    if (!userEquipment || userEquipment.length === 0) return exercisesByMuscle;
    const out: Record<string, FirestoreExercise[]> = {};
    for (const m of Object.keys(exercisesByMuscle)) {
      const filtered = exerciseService.filterByUserEquipment(exercisesByMuscle[m], userEquipment);
      // Safety: if strict filter empties the list, fall back to unfiltered so the UI
      // never looks "broken" when a user's saved equipment doesn't cover a muscle.
      out[m] = filtered.length > 0 ? filtered : exercisesByMuscle[m];
    }
    return out;
  }, [exercisesByMuscle, equipKey]); // eslint-disable-line react-hooks/exhaustive-deps
          ...
          <div key={muscle}>
            ...
            <div className="flex items-center space-x-2 py-1.5 sticky top-0 bg-slate-950 z-10 border-b border-slate-800 mb-2">
              ...
              {isLoading
                ...
                : <span className="text-[9px] text-slate-600 font-mono">{exercises.length} exercises</span>
              }
            </div>
            ...
            {!isLoading && exercises.length > 0 && (
              <div className="space-y-1.5">
                {exercises.map(ex => {
                  ...
                  const uiMuscle = mapDBToMuscleGroup(ex.targetMuscle, muscle);
                  const exDiff = calculateDifficulty(ex);
                  const exXP = exDiff * 10;
                  // Build granular muscle list for premium thumbnail
...
export const GymTracker: React.FC = () => {
  ...
  /** FIX: startPackage was always returning early because exercises array was empty. */
  const startPackage = (pkg: WorkoutRoutine) => {
    setSelectedMuscles(pkg.muscleGroups);
    // Navigate to select exercises step; ExerciseBrowser will auto-fetch for these muscles
    setSelectedExercises([]);
    setFlowStep('selectExercises');
  };
        ...
        <div className="space-y-6">
          ...
          <div className="jarvis-card p-6 rounded-xl flex flex-col items-center justify-center text-center space-y-4 shadow-lg border border-slate-700/50 relative overflow-hidden">
            ...
            <div className="relative flex justify-center items-center">
              {calculateStreak(logs) > 0 && <div className="absolute inset-0 bg-orange-500/20 blur-xl rounded-full scale-150 animate-pulse" />}
              <Flame size={72} strokeWidth={1.5} className={`z-10 transition-all duration-1000 ${calculateStreak(logs) > 0 ? 'text-orange-500 drop-shadow-[0_0_25px_rgba(249,115,22,0.9)] scale-110 animate-bounce' : 'text-slate-700'}`} />
            </div>
            <div className="z-10 mt-2">
              <span className="text-6xl font-extrabold text-transparent bg-clip-text bg-gradient-to-br from-white to-slate-300 font-mono tracking-tighter">{calculateStreak(logs)}</span>
            ...
            <p className="text-sm text-slate-400 max-w-[250px] z-10 mt-2 font-medium">
              {calculateStreak(logs) > 0 ? "Keep the fire burning! Don't break the streak! 🔥" : 'Time to ignite your streak! Start a workout today.'}
            </p>
          </div>
